function [newFlagPeopleNear,count, periods] = countMeetPeople(rawFlagSeePeople)
%countMeetPeople finite state machine to determin the 'real' number of
%times people are near the robot.
breaklen = 0;
delayer = 3;
numberOfPeopleNear = 0;
newFlagPeopleNear = rawFlagSeePeople;
changeFlag = false;
for i=2:length(rawFlagSeePeople(:,1))
    rawFlagSeePeople(i,1) = 0.5 * rawFlagSeePeople(i,1) + 0.5 * rawFlagSeePeople(i-1,1);
    if rawFlagSeePeople(i,1) > 0.125
        newFlagPeopleNear(i-1:i,1) = 1;
        numberOfPeopleNear = numberOfPeopleNear + 1;
    else
        newFlagPeopleNear(i-1:i,1) = 0;
        rawFlagSeePeople(i-1:i,1) = 0;
    end
end
threshold = 5;
